...
全部填完后提交。主试可在后台查看进度、复制被试链接、导出数据、修改指导语与账号。
...
| 角色 | 能做什么 |
|------|----------|
| **流水线**（VAD / ASR / EasyTurn） | 任务结束后 `POST` 创建会话，拿到被试链接 |
...
1. 点某行 **详情**
2. 页面上方 **「被试链接」** 输入框里就是完整 URL
3. 点 **复制链接**（或 **复制被试链接**）
4. 若浏览器禁止自动复制：链接会处于选中状态，用 **Ctrl+C** / 手机长按复制
5. 把链接发给被试（微信、邮件等）
...
1. 主试在详情页复制  
2. 或流水线创建会话后打印的 `participant_url`
...
- [ ] 流水线或 `pipeline_client.py` 能创建会话，`target_count` 正确
- [ ] 主试详情页能看到并复制被试链接
